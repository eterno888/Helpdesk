...
Route::group(['middleware' => ['auth', 'userLocale']], function () {
    ...
    Route::group(['middleware' => 'can:see-admin'], function () {
        ...
        Route::get('membership/{user}', 'TeamMembershipController@index')->name('users.membership');
        Route::post('membership/{user}/store', 'TeamMembershipController@store')->name('users.membership.store');
